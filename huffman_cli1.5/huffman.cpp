...
bool CompressFile(const std::filesystem::path& inputPath,
    ...
    std::ifstream in;
    in.open(inputPath.c_str(), std::ios::binary);
    if (!in) {
        std::cerr << "Error: cannot open input file: " << inputPath.string() << "\n";
        return false;
    }
    ...
    std::ofstream out;
    out.open(outputPath.c_str(), std::ios::binary);
    if (!out) {
        std::cerr << "Error: cannot create output file: " << outputPath.string() << "\n";
        return false;
    }
...
bool DecompressFile(const std::filesystem::path& inputPath,
    ...
    if (!lengths.valid) {
        std::cerr << "Error: invalid code table in input file.\n";
        return false;
    }
    ...
    if (decoded != originalSize) {
        std::cerr << "Error: decompressed size mismatch (" << decoded << " vs " << originalSize << ")\n";
    }
...
bool CompressFiles(const std::vector<std::filesystem::path>& filePaths,
    ...
    if (allFiles.empty()) {
        std::cerr << "Error: no files to compress.\n";
        return false;
    }
    ...
    for (size_t i = 0; i < allFiles.size(); ++i) {
        ...
        if (ec) {
            std::cerr << "Warning: cannot get size of " << allFiles[i].string() << ": " << ec.message() << "\n";
            entries[i].originalSize = 0;
        }
    }
    ...
    for (const auto& e : entries) {
        std::string utf8path = e.relativePath.u8string();
        headerSize += 2 + static_cast<uint32_t>(utf8path.size()) + 8 + 8 + 8;
    }

    std::ofstream out;
    out.open(archivePath.c_str(), std::ios::binary);
    if (!out) {
        std::cerr << "Error: cannot create archive file: " << archivePath.string() << "\n";
        return false;
    }
    ...
    for (size_t i = 0; i < allFiles.size(); ++i) {
        std::ifstream inFile;
        inFile.open(allFiles[i].c_str(), std::ios::binary);
        if (!inFile) {
            std::cerr << "Error: cannot open input file: " << allFiles[i].string() << "\n";
            return false;
        }
    ...
    if (!out.good()) {
        std::cerr << "Error: output stream error after writing header.\n";
        return false;
    }
    return true;
}
...
bool DecompressArchive(const std::filesystem::path& archivePath,
                       ...
                       ProgressCallback progress) {
    std::ifstream in;
    in.open(archivePath.c_str(), std::ios::binary);
    if (!in) {
        std::cerr << "Error: cannot open archive: " << archivePath.string() << "\n";
        return false;
    }
    ...
    if (magic != ARCHIVE_MAGIC) {
        std::cerr << "Error: invalid archive format (bad magic number).\n";
        return false;
    }
    uint32_t fileCount;
    ...
    for (uint32_t i = 0; i < fileCount; ++i) {
        ...
        entries[i].relativePath = std::filesystem::u8path(utf8path);
    ...
    for (uint32_t i = 0; i < fileCount; ++i) {
        ...
        if (!outFile) {
            std::cerr << "Warning: cannot create output file: " << outPath.string() << "\n";
            continue;
        }
        if (!lengths.valid) {
            outFile.close();
